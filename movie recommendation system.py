...
with open("Resource-File/anindita-ratings.csv", newline='') as csvFile:
    ...
    for row in ratings_reader:
        ...
        if(row_ratings != "") and (float(row_ratings) > 0) and (float(row_ratings) < 6):
            anindita_ratings_dict.update({int(row["item"]): float(row_ratings)})
...
anindita_recommender = algo.recommend(-1, num_represent, ratings=pd.Series(anindita_ratings_dict))
joined_data = anindita_recommender.join(data_set.movies["genres"], on="item")
joined_data = joined_data.join(data_set.movies["title"], on="item")
joined_data = joined_data[joined_data.columns[2:]]
print("Recommendation For Anindita: ")
print("")
...
for k in anindita_ratings_dict:
    if k in my_friend_ratings_dict:
        combined_rating_dict.update({k: float((anindita_ratings_dict[k] + my_friend_ratings_dict[k])/2)})
    else:
        combined_rating_dict.update({k: anindita_ratings_dict[k]})

for k in my_friend_ratings_dict:
    if k in anindita_ratings_dict:
        combined_rating_dict.update({k: float((my_friend_ratings_dict[k] + anindita_ratings_dict[k])/2)})
    else:
...
joined_data = joined_data.join(data_set.movies["title"], on="item")
joined_data = joined_data[joined_data.columns[2:]]
print("Recommendation Of Movies For Both Anindita And Her Friend: ")
print("")
print(joined_data.to_string())
print("")
print("---------------------------------------------------------------------------------------------------------------")
